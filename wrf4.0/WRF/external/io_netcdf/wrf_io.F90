...
subroutine ext_ncd_inquire_filename( Datahandle, FileName,  FileStatus, Status )
  ...
  type(wrf_data_handle) ,pointer        :: DH
  FileStatus = WRF_FILE_NOT_OPENED
  WRITE(msg, FMT='(A, I10)') 'ZFY20181205 ./external/io_netcdf/wrf_io.F9003 ', DataHandle
  CALL wrf_debug( 1000, TRIM(msg) )
  call GetDH(DataHandle,DH,Status)
  WRITE(msg, *) 'ZFY20181206 ./external/io_netcdf/wrf_io.F9004 ', trim(DH%FileName) !DH%CurrentTime
  CALL wrf_debug( 1000, msg )
  WRITE(msg, FMT='(A, I10)') 'ZFY20181205 ./external/io_netcdf/wrf_io.F9002 ', DataHandle
  CALL wrf_debug( 1000, msg )
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'Warning Status = ',Status,' in ',__FILE__,', line', __LINE__
    call wrf_debug ( WARN , TRIM(msg))
  ...
  WRITE(msg, FMT='(A, A)') 'ZFY20181205 ./external/io_netcdf/wrf_io.F9001 ', TRIM(FileName)
  CALL wrf_debug( 1000, TRIM(msg) )
  FileStatus = DH%FileStatus
  Status = WRF_NO_ERR
  return
